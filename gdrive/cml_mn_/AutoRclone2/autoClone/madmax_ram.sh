        ...
        if [ ! -z $MADMAX_PROCESS_ID ]
            then
				...
				then
						...
						screen -dmS  clone
						screen -S clone -X stuff  "/root/AutoRclone/autoClone/autoClone.sh disk2 $1 $3 $4 $7 ^M"
#						screen -S clone -X stuff  "/root/AutoRclone/autoClone/autoClone.sh disk2 $1 $2 $3 $4 ^M"
                ...
                then
                        ...
                        sudo mount -t tmpfs -o size=$6G tmpfs /disk2/temp2/
                        rm -r /disk2/temp/*

                        screen -ls | awk -vFS='\t|[.]' '/tavuk/ {system("screen -S "$2" -X quit")}'
                        screen -dmS tavuk
                        screen -S tavuk -X stuff "cd /root/chia-plotter^M" 
                        screen -S tavuk -X stuff "./build/chia_plot -n -1 -r $5  -t /disk2/temp/ -2 /disk2/temp2/ -d /disk2/plots/ -c xch1wxvh0lupku627a3qnnrggssr9ey4utcfmk87llvg94wgahemc2yq2u37za -f b87d02b56e479832e7414bd7a0d8c3b03f18947eb406d82c9b1de30fcbad012fb84fe7bbd0b335310bcfd344a8bd9e94 ^M"
        fi
